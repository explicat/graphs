...
public class FordFulkerson {
    ...
    public static List<DirectedEdge> bfs(final Diagraph diagraph, final int source, final int target) {
        return bfs(diagraph, source, target, false);
    }

    public static List<DirectedEdge> bfs(final Diagraph diagraph, final int source, final int target, boolean checkForRemainingCapacity) {
        int nodes = diagraph.nodes();
        ...
        while(!queue.isEmpty() && !foundTarget) {
            ...
            for (DirectedEdge e : edges) {
                if (!checkForRemainingCapacity || (e.capacity() - e.flow() > 0)) {
                    int v = e.to();
                    if (target == v) {
                        parent.put(v, u);
                        foundTarget = true;
        ...
        while (predecessor >= 0) {
            ...
            path.add(possibleEdges.iterator().next());
            current = predecessor;
    ...
    public static Diagraph fordFulkerson(final Diagraph diagraph, final int source, final int sink, int maxFlow) {
        ...
        List<DirectedEdge> path;
        while (null != (path = bfs(residual, source, sink, true)) && (curFlow < maxFlow)) {
            // Find maximum remaining capacity along edges. We cannot move more flow than the minimum available remaining capacity
            int pathFlow = Integer.MAX_VALUE;
            for (DirectedEdge e : path) {
                pathFlow = Math.min(pathFlow, (e.capacity() - e.flow()));
